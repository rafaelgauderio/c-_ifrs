...
int main() {
	...
	ObjBase *objArray3[3] = {objeto1,objetoInteiro,objetoDecimal};
    ...
    for (int j = 0; j <3 ; j++)
    {
    std::cout << "deleting object of Array " << typeid(*objArray[j]).name() << endl;
    delete objArray[j];
    }
    ...
    for (int j = 0; j <3 ; j++)
    {
    std::cout << "deleting object of Array2 " << typeid(*objArray2[j]).name() << endl;
    //delete objArray2[j];
    }
